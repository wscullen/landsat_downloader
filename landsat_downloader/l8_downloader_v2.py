...
class L8Downloader:
    ...
    def __init__(self, path_to_config='config.yaml', username=None, password=None, verbose=False):
        ...
        try:
            ...
            self.logger.error(f'Unexpected problem occurred while loading config ({str(e)})')
    ...
    def dataset_search(self, dataset_name_to_search):
        ...
        if api_key:
            ...
            try:
                ...
                self.logger.error(f'There was a problem with the request. Exception: {str(e)}')
                raise e
            else:
                result = r.json()
                ...
                if r.status_code == 200:
                    ...
                    if result['errorCode']:
                        self.logger.warning(f"There was a problem with the request, error: {result['errorCode']}, errorMessage: {result['errorMessage']}")
                    else:
                        return result['data']
    ...
    def scene_search(self, dataset_name, 
        ...
        if api_key:
            ...
            if scene_name_list:
                self.logger.info('Metadata filters are not operational at this time, scene_name_list ignored...')
                metadata_filter = {
                    "filterType": "or",
                    "childFilters": None
                }
            ...
            if not scene_name_list and not (upper_right and lower_left or geojson):
                self.logger.info('Missing essential search parameters, aborting search.')
                return None
    ...
    def format_results(self, raw_results, platform_name):
        ...
        if platform_name == 'Landsat-8':
            ...
            for result in raw_results:
                ...
                thumbnail_url = [browse["browsePath"] for browse in result["browse"] if browse["browseName"] == "LandsatLook Natural Color Preview Image"][0]
                hq_preview_url = [browse["browsePath"] for browse in result["browse"] if browse["browseName"] == "LandsatLook Quality Preview Image"][0]

                product_dict['thumbnail_url'] = thumbnail_url
                product_dict['hq_preview_url'] = hq_preview_url
                product_dict['options'] = [key for key in result["options"].keys() if result["options"][key]]
                ...
                path = [metadata_entry["value"].strip() for metadata_entry in result["metadata"] if metadata_entry["fieldName"] == "WRS Path"][0]
                row = [metadata_entry["value"].strip() for metadata_entry in result["metadata"] if metadata_entry["fieldName"] == "WRS Row"][0]
                ...
                land_cloud = [metadata_entry["value"].strip() for metadata_entry in result["metadata"] if metadata_entry["fieldName"] == "Land Cloud Cover"][0]
                scene_cloud = [metadata_entry["value"].strip() for metadata_entry in result["metadata"] if metadata_entry["fieldName"] == "Scene Cloud Cover"][0]

                product_dict['land_cloud_percent'] = land_cloud
                product_dict['scene_cloud_percent'] = scene_cloud

                utm_zone = [metadata_entry["value"] for metadata_entry in result["metadata"] if metadata_entry["fieldName"] == "UTM Zone"][0]
